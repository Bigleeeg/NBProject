...
namespace MailTest.TestApplication
{
    ...
    public class TestForm : Form
    {
        ...
        private void btnSendMail_Click(object sender, EventArgs e)
        {
            try
            {
                //OutMailDB.WriteLog("OnStart", "进入成功!");
        ...
        private void SendMail(ServerConf sc)
        {
            ...
            if (sc.SendConfiguration.Enabled)
            {
                ...
                DataSet ds = OutMailDB.GetMailByStatus(OutMailStatus.UnSend, mailConf.FailRetryIntervalPow, mailConf.FailMostNum, sc.Provider.ConnectionString);//根据当前的数据库链接  查看未发送邮件的信息                 
                ...
                foreach (string strID in senderIDs)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        if (sc.SendConfiguration.LogConfiguration.Enabled)
                            OutMailDB.WriteLog("SendMail#####Exception", "要发送邮件SendMailID:" + strID + ";Ex:" + ex.Message, sc.SendConfiguration.LogConfiguration.Url);
                        if (int.Parse(drs[0]["SendCount"].ToString()) >= mailConf.FailMostNum - 1)
                        {
                            ...
                            //LogManager.LogError("邮件发送失败", ex.Message, drs[0]["MailToAddress"].ToString(), drs[0]["MailToAddress"].ToString(), Configuration.LogCategoryID, Configuration.LogCategoryName);
                        }
                        //失败 根据rc.ConnectionString来修改
